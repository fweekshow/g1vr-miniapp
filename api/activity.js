  ...
  try {
    ...
    const activitiesWithRealSenders = await Promise.all(
      activityData.map(async (activity) => {
                ...
                if (log.topics && log.topics.length >= 2) {
                  ...
                  if (addressTopic && addressTopic.startsWith('0x000000000000000000000000')) {
                    try {
                      ...
                      foundAddress = accountAddress;
                      break; // Exit the inner loop once we find the address
                    } catch (error) {
                      console.error('Error processing address topic:', error, 'Topic:', addressTopic);
                    }
                  }
                }
              }
              if (foundAddress) {
                return { ...activity, actualSender: foundAddress };
              }
            }
          } catch (error) {
        ...
        // If no API key or extraction failed, return activity as-is
        return activity;
      })
    );
